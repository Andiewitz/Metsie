...
export default function PlayPage() {
  ...
  return (
    <div className="absolute inset-0 flex flex-col justify-between overflow-y-auto no-scrollbar py-2 px-6 sm:px-10 lg:px-12">
      {/* ── TOP: CS2 Mode Sub-Header ─────────────────────────────── */}
      <div className="shrink-0 flex flex-col items-center gap-2 pt-1 pb-3">
        {/* Row 1: Primary Mode Buttons (MATCHMAKING | PRACTICE | Globe) */}
        <div className="flex items-center gap-2">
          <button className="px-5 py-1.5 rounded-lg border border-cyan-400/60 bg-cyan-500/20 text-cyan-300 text-xs font-black tracking-wider uppercase font-small shadow-[0_0_15px_rgba(34,211,238,0.3)] cursor-pointer">
            MATCHMAKING
          </button>
          <button className="px-5 py-1.5 rounded-lg border border-white/10 bg-black/30 hover:bg-white/10 text-zinc-400 hover:text-zinc-200 text-xs font-black tracking-wider uppercase font-small transition cursor-pointer">
            PRACTICE
          </button>
          <button
            className="p-1.5 rounded-lg border border-white/10 bg-black/30 hover:bg-white/10 text-zinc-400 hover:text-zinc-200 transition cursor-pointer"
      ...
      {/* ── CENTER: CS2 9-Map Grid (Subjects as Maps) ─────────────── */}
      ...
      <div className="shrink-0 pt-3 pb-2 flex items-center justify-between gap-4 flex-wrap border-t border-white/10 backdrop-blur-md bg-zinc-950/40 rounded-2xl px-6 py-2.5 mt-2 shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)]">
        {/* Selected count info */}
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.9)] animate-pulse" />
            <span className="text-xs font-black text-white font-main uppercase tracking-wider">
              {selectedMapIds.length} {selectedMapIds.length === 1 ? "Subject" : "Subjects"} Selected
            </span>
          </div>
          <span className="text-zinc-600">|</span>
          <span className="text-xs text-zinc-300 font-sub">
            Estimated wait time: <span className="text-amber-300 font-mono font-bold">~01:45</span>
          </span>
        </div>
        ...
        {!isMatchmaking ? (
          <button
            ...
            className="group/go relative px-10 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 disabled:opacity-40 disabled:cursor-not-allowed text-zinc-950 font-black text-sm font-main tracking-widest uppercase border border-emerald-300/60 shadow-[inset_0_1px_1px_rgba(255,255,255,0.7),0_8px_25px_rgba(16,185,129,0.45)] hover:shadow-[inset_0_1px_1px_rgba(255,255,255,0.9),0_12px_35px_rgba(16,185,129,0.6)] hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center gap-2.5 cursor-pointer"
          >
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-xs font-black text-cyan-300 font-main uppercase tracking-wider">
                Searching... ({formatTime(queueTime)})
              </span>
            </div>
            <button
              onClick={handleCancelMatchmaking}
              className="px-5 py-1.5 rounded-xl border border-rose-500/40 bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 text-xs font-black font-main uppercase tracking-wider transition cursor-pointer"
            >
              CANCEL
            </button>
          </div>
...
/**
 * Single Map Card component replicating the exact CS2 card anatomy from screenshot
 */
function MapCard({
  subject,
  isSelected,
  ...
  return (
    <div
      onClick={onToggle}
      className={`group/card relative h-56 sm:h-64 rounded-2xl p-3 flex flex-col justify-between overflow-hidden cursor-pointer select-none transition-all duration-200 ${
        isSelected
          ? "border-2 border-cyan-400/90 shadow-[0_0_20px_rgba(34,211,238,0.35),inset_0_1px_1px_rgba(255,255,255,0.4)] scale-[1.02] brightness-105"
          : "border border-white/15 hover:border-white/40 shadow-lg hover:brightness-105"
      }`}
    >
      ...
      <div className="relative z-10 flex items-center justify-between">
        {/* CS-style mini runner badge */}
        <div className="w-5 h-5 rounded-md bg-amber-400/90 border border-amber-300/80 flex items-center justify-center text-zinc-950 font-black text-[10px] shadow-sm">
          <span className="leading-none">{subject.iconLetter}</span>
        </div>

        {/* CS2 Map selection checkbox */}
        <div
          className={`w-5 h-5 rounded-md border flex items-center justify-center transition-colors ${
            isSelected
              ? "border-cyan-400 bg-cyan-500/80 text-zinc-950 shadow-[0_0_8px_rgba(34,211,238,0.8)]"
              : "border-white/30 bg-black/40 group-hover/card:border-white/60"
          }`}
        >
      ...
      {/* ── CENTER: Big Circular/Shield Emblem Badge ── */}
      <div className="relative z-10 flex flex-col items-center my-auto">
        <div
          className={`w-20 h-20 sm:w-22 sm:h-22 rounded-full border-2 bg-gradient-to-b ${subject.badgeBg} ${subject.badgeColor} flex items-center justify-center shadow-[0_8px_20px_rgba(0,0,0,0.6),inset_0_1px_1px_rgba(255,255,255,0.4)] group-hover/card:scale-105 transition-transform duration-200`}
        >
          {subject.emblemIcon}
        </div>

        {/* [ ? ] Unranked / Mastery Box under emblem */}
        <div className="mt-2 px-3 py-0.5 rounded-md bg-black/60 border border-white/20 text-[10px] font-black text-zinc-400 font-mono tracking-wider shadow-inner">
          ?
        </div>
      </div>
      ...
      <div className="relative z-10 text-center pt-1 border-t border-white/10">
        <h3 className="text-xs sm:text-sm font-black text-white font-main tracking-wide truncate drop-shadow-[0_1px_3px_rgba(0,0,0,0.9)]">
          {subject.name}
        </h3>
