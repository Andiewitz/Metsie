  ...
  return (
    <div className="fixed inset-0 flex flex-col overflow-hidden select-none bg-zinc-950">
      ...
      <header className="relative z-50 flex items-center justify-between h-12 px-4 shrink-0 backdrop-blur-xl bg-zinc-950/60 border-b border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.3)]">
        ...
        {/* Right side spacer to keep center tabs perfectly balanced */}
        <div className="w-16 shrink-0" />
      </header>

      {/* ── Body: Main Canvas (Offset by sidebar width: mr-16) ── */}
      <div className="flex-1 flex overflow-hidden relative mr-16">
        <main className="flex-1 relative overflow-hidden">
          {children}
        </main>
      </div>

      {/* ── Sidebar: Fixed to right side, full height (top-0 bottom-0) sitting higher at header level with z-50 ── */}
      <ProfileSidebar />
    </div>
  )
}
